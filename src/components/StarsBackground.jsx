...
export default function StarsBackground() {
  ...
  'rgba(180, 255, 230, OP)'    // vert très clair presque blanc
];
    ...
    const createStars = (count) => {
      for (let i = 0; i < count; i++) {
        ...
        const color = colors[Math.floor(Math.random() * colors.length)].replace('OP', opacity);
        ...
        // Avant : const duration = 20 + Math.random() * 60; // 20 à 80s
const duration = 60 + Math.random() * 60; // 60 à 120s, beaucoup plus lent
star.style.animation = `fallStar ${duration}s linear infinite`;
        ...
        // Stock star pour cleanup
        container.appendChild(star);
        stars.push({ node: star, offsetX });
      }

      // Animation via requestAnimationFrame pour trajectoire inclinée
      let startTime = null;
      const animate = (timestamp) => {
        ...
        stars.forEach(({ node, offsetX }) => {
          ...
          if (top > 100) top = -5; // reset top
          node.style.top = `${top}%`;
          // mouvement horizontal léger
          node.style.transform = `translateX(${Math.sin(elapsed + offsetX) * 20}px)`;
        });
